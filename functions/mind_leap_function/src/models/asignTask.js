...
class asignTaskModel {
  static async getAllbyUT(req,  idUT ) {
    ...
    const query= `SELECT taskInfo.title, taskInfo.description, taskInfo.isPublic, taskInfo.video,
...
task.image, task.description, task.video from userTutee
...
//QUERIES FOR ROWIDS 
  ...
  static async delete( req, idTask , idUT ) {
    ...
    // const [asignTask] = await connection.executeZCQLQuery(`SELECT * FROM asingn_task WHERE idTask = ? and idUT = ?;`, [ idTask , idUT]);
    
    // const findAsignTask =  asignTask[0]

    // if ( !findAsignTask ) throw new Error('asignTask no found');
    
    // const [result] = await connection.executeZCQLQuery(`DELETE FROM asingn_task WHERE idTask = ? and idUT = ?;`, [ idTask , idUT]);
